var hid_open = function() {
  
  };
  
var hid_handle = false;

var HE853Controller = {

  init : function() {
  	anban_cnt = 0;
  	hid_handle = hid_open(0x4d9, 0x1357, NULL);
  	if (!hid_handle) {
  		console.log("Unable to open device\n");
  	  }
    },

  sendOutputReport : function(buf) {
    var obuf = new Buffer(9);
    obuf.write(0x00); // report id = 0, as it seems to be the only report

   	for (var i = 0; i < 8; i++) {
   		obuf[i + 1] = buf[i];
   	}
   	return(hid_write(hid_handle, obuf, 9));
    },

  readDeviceStatus : function() {
    var buf = new Buffer(8);
	  // memset(buf, 0x00, sizeof(buf)); // don't know if that will be necessary
	  buf[0] = 0x06;
	  buf[1] = 0x01;
	  return(sendOutputReport(buf));
	  },
	  
  sendRfData_GER : function(deviceCode, cmd) {
    //var dcb = new Buffer(2);
    //dcb.writeUInt16LE(jDeviceCode);
  
    var rfCmdBuf = new Buffer(32);
	  var i = 0;
	  
	  var tb_fx = new Buffer(0x07, 0x0b, 0x0d, 0x0e,
    		  	       0x13, 0x15, 0x16, 0x19,
			             0x1a, 0x1c, 0x03, 0x05,
			             0x06, 0x09, 0x0a, 0x0c);
    ...
    buf[0] = 0x00;
    buf[1] = (deviceCode >> 8) & 0xff;
    buf[2] = (deviceCode & 0xff);
    buf[3] = 0x00;

	  if (cmd == true)
		  buf[3] = buf[3] | 0x10;

    var gbuf = new Buffer(8);
    gbuf[0] = buf[0] >> 4;
    gbuf[1] = buf[0] & 15;
    gbuf[2] = buf[1] >> 4;
    gbuf[3] = buf[1] & 15;
    ...
    var kbuf = new Buffer(8);
  	for (var i = 0; i < 8; i++) {
		  kbuf[i] = (tb_fx[gbuf[i]] | 0x40) & 0x7f;
	    }
	  kbuf[0] = kbuf[0] | 0x80;
