...
messageHandlers = {
  ...
  busmessage : function(data) {  
    if(busDataSubscribers[data.data.device]) 
      busDataSubscribers[data.data.device](data.data);
  },
...
wsConnect = function() {
  ...
  ws = new WebSocket('ws://<?= cfg('service/wserverurl') ?>/ws');
  ws.onopen = function() {
